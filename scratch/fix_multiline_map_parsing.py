...
if match:
    ...
    for header, body, footer in lang_blocks:
        ...
        # Parse all key-value entries 'key': 'val'
        entries = re.findall(r"'([^']+)'\s*:\s*('([^'\\]*(?:\\.[^'\\]*)*)')", body)
        for k, v_quoted, raw_v in entries:
            if k in seen_keys:
                continue
            seen_keys.add(k)
            rebuilt_map.append(f"      '{k}': {v_quoted},")
    ...
    print("Rebuilt app_localizations.dart with clean single-entry lines!")
